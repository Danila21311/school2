...
export const Route = createFileRoute("/test")({
  beforeLoad: async () => {
    ...
    return { enabled: access.filter((x) => x.content_type === "test" && x.is_enabled === 1) };
  },
  component: TestPage,
});
    ...
    <PageShell>
      <section className="mx-auto max-w-5xl">
        <div className="rounded-[2rem] border border-white/70 bg-white/85 p-8 shadow-xl backdrop-blur sm:p-10">
          <h1 className="font-display text-3xl font-extrabold">Промежуточные тесты</h1>
          {available.length === 0 ? (
            <p className="mt-4 text-muted-foreground">Преподаватель пока не открыл вам доступ к тестам.</p>
          ) : (
            <>
